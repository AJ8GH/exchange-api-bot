...
public class AccountAPINGException extends RuntimeException {
  ...
  public AccountAPINGException(String message, ErrorCode errorCode, String errorDetails,
      String requestUUID) {
    super(message);
    this.errorCode = errorCode;
    this.errorDetails = errorDetails;
  ...
  public enum ErrorCode {
    ...
    USER_NOT_SUBSCRIBED(
        "The user making the request is not subscribed to the Application Key. they are trying to perform the action on (e.g. creating an Authorisation Code)."),
    INVALID_SECRET(
    ...
    INVALID_GRANT_TYPE(
        "The vendor making the request has not provided a valid grant_type, or the grant_type they have passed does not match the parameters (authCode/refreshToken)"),
